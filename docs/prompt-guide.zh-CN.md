...
- 风险回报比 ≥ 1:3
- 最多持仓 3 个币种
- 最大杠杆由策略配置（最高 125x），执行时还会按交易所当前逐标的杠杆档位进一步限制
- 保证金使用率 ≤ 90%
...
#### 4. 杠杆限制
**要求**:
- 所有标的: 不得超过策略配置的最大杠杆（策略上限最高 125x）
- 执行层: 还会查询交易所当前逐标的杠杆档位，并采用两者中的较低值
...
杠杆使用：
- 山寨币: 4~5x 杠杆（接近上限）
- BTC/ETH: 15~20x 杠杆（高风险区间，仍不得超过策略与交易所上限）
...
4. 杠杆: 不得超过策略配置上限，也不得超过交易所当前逐标的档位
5. 保证金使用率 ≤ 90%
