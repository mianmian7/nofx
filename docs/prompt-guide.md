...
- BTC/ETH: 15~20x leverage (high-risk range; still capped by strategy and exchange limits)
...
4. Leverage: never exceed either the strategy maximum or the exchange's current per-symbol bracket
5. Margin usage ≤ 90%
...
**Common Causes**:
- Risk-reward ratio < 1:3
- Leverage exceeds the strategy maximum or the exchange's current per-symbol bracket
